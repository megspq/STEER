...
const levelInfo = {
  easy:   { song:"月亮代表我的心", level:"第 1 关 · 简单", icon:"月", coverClass:"easy" },
  medium: { song:"甜蜜蜜",         level:"第 2 关 · 中等", icon:"甜", coverClass:"medium" },
  hard:   { song:"许多年以后",     level:"第 3 关 · 困难", icon:"许", coverClass:"hard" }
};

// ── UI HELPERS ─────────────────────────────────────────────────────────────
function updateNowPlaying() {
  const info = levelInfo[selectedDifficulty];
  if (currentSongName)  currentSongName.innerText  = info ? info.song  : "请选择难度";
...
function showFeedback(type) {
  ...
  speechText.innerText = type === "perfect" ? "真棒!" : "加油!";
...
async function startGame() {
  if (gameRunning) return;
  if (!selectedDifficulty) { statusMessage.innerText = "请选择难度"; return; }
...
function selectLevel(level) {
  ...
  statusMessage.innerText = "已选择第 " + level + " 关。请按开始";
}
...
function restartSameLevel() {
  ...
  statusMessage.innerText = selectedDifficulty + " — 请按开始";
}
...
function pickPrize() {
  ...
  if (rewardText) rewardText.innerText = "恭喜获得: " + r;
